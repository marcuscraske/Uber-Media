...
namespace UberCMS.Installer
{
    public class InstallScript
    {
        public static void install(Connector conn)
        {
            ...
            // Install ubermedia
            string umPluginid = null;
            if((error = Misc.Plugins.install(null, ref umPluginid, basePath + "\\App_Code\\Plugins\\Uber Media", false, conn)) != null)
                throw new Exception("Failed to install ubermedia plugin: " + error);
            if((error = Misc.Plugins.enable(umPluginid, conn)) != null)
                throw new Exception("Failed to enable ubermedia plugin: " + error);
        }
    }
}