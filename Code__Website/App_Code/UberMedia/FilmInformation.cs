...
namespace UberMedia
{
    ...
    public static class FilmInformation
    {
        ...
        public static void cacheStart_Thread()
        {
            ...
            Result res = Core.GlobalConnector.Query_Read("SELECT cache_updated FROM film_information_providers WHERE title='" + Utils.Escape(PROVIDER_IDENTIFIER_IMDB) + "'");
        ...
        {
            if (!Core.Cache_Settings.ContainsKey("rotten_tomatoes_api_key") || Core.Cache_Settings["rotten_tomatoes_api_key"].Length == 0) return null;
            try
            {
                System.Threading.Thread.Sleep(100); // To ensure we dont surpass the API calls per a second allowed
                string url = "http://api.rottentomatoes.com/api/public/v1.0/movies.json?q=" + HttpUtility.UrlEncode(title) + "&page_limit=3&page=1&apikey=" + Core.Cache_Settings["rotten_tomatoes_api_key"];
                ...
                else
                {
                    ...
                    int provid;
                    object provRaw = conn.Query_Scalar("SELECT provid FROM film_information_providers WHERE title='" + Utils.Escape(PROVIDER_IDENTIFIER_RT) + "'");
                    if (provRaw == null)
                        provid = int.Parse(conn.Query_Scalar("INSERT INTO film_information_providers (title) VALUES('" + Utils.Escape(PROVIDER_IDENTIFIER_RT) + "'); SELECT LAST_INSERT_ROW();").ToString());
                    else
                        provid = int.Parse(provRaw.ToString());
                    // -- Insert the data
                    conn.Query_Execute("INSERT INTO film_information (title, description, provid) VALUES('" + Utils.Escape(title) + "', '" + Utils.Escape(returnData) + "', '" + provid + "')");
                    return returnData;
                }
            }
            catch
        ...
        {
            Connector imdbConn = Core.Connector_Create(true);
            string PATH_CACHE = AppDomain.CurrentDomain.BaseDirectory + "/Cache";
            ...
            try
            {
                ...
                else
                {
                    ...
                    imdbConn.Query_Execute("DELETE FROM film_information WHERE provid='" + provid + "'");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                const string STATEMENT_START = "INSERT INTO film_information (title, description, provid, last_updated) VALUES";
            ...
            imdbConn.Query_Execute("UPDATE film_information_providers SET cache_updated=NOW() WHERE provid='" + provid + "'");
        }
        #endregion
